...
kubectl --context "${K8S_CONTEXT}" get namespace "${NS}" >/dev/null 2>&1 ||   kubectl --context "${K8S_CONTEXT}" create namespace "${NS}" >/dev/null 2>&1 || true

kubectl --context "${K8S_CONTEXT}" -n "${NS}" delete job   -l "bench=faas-hybrid-etl,run_id=${RUN_ID},partition_id=${PARTITION_ID}"   --ignore-not-found=true >/dev/null 2>&1 || true
...
for ((shard_id=0; shard_id<${TOTAL_SHARDS}; shard_id++)); do
  if (( shard_id % WORKERS != PARTITION_ID )); then
    continue
  fi

  assigned=$((assigned + 1))
  job_name=$(printf "faas-b2-hybrid-%s-w%d-s%d" "${RUN_ID}" "${PARTITION_ID}" "${shard_id}" | tr '[:upper:]' '[:lower:]')
...
EOF
  ...
  status="ok"
  if ! kubectl --context "${K8S_CONTEXT}" -n "${NS}" wait       --for=condition=complete       --timeout="${JOB_TIMEOUT}"       "job/${job_name}" >/dev/null; then
    status="failed"
  fi
...
{"benchmark":"faas_hybrid_etl_v1","run_id":"${RUN_ID}","stack":"${STACK}","partition_id":${PARTITION_ID},"shard_id":${shard_id},"job_name":"${job_name}","node_name":"${NODE_NAME}","t_submit_wall":${t_submit},"t_done_wall":${t_done},"job_wall_ms":${job_wall_ms},"status":"${status}"}
EOF

  if [[ "${KEEP_NAMESPACE}" != "1" ]]; then
    kubectl --context "${K8S_CONTEXT}" -n "${NS}" delete job "${job_name}"       --ignore-not-found=true >/dev/null 2>&1 || true
  fi
done
