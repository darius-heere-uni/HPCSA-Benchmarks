#!/usr/bin/env python3
"""
Aggregate Benchmark 2 hybrid ETL results from both stacks.

Expected tree:
<results_root>/
  k8s/
    bench-b2-hybrid-k8s-r01/
      part_00/
      part_01/
    ...
  ksi/
    bench-b2-hybrid-ksi-r01/
      part_00/
      part_01/
    ...
...
- out_*.json           -> in-container function timing and ETL aggregates

Writes:
- benchmark2_run_summary.csv
- benchmark2_part_summary.csv
- benchmark2_job_latencies.csv
- benchmark2_function_times.csv
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate Benchmark 2 hybrid ETL results.")
    parser.add_argument(
        "--results-root",
        default="/nfs/bench/faas_hybrid_etl_v1/results",
        help="Root directory containing k8s/ and ksi/ result folders.",
    )
    parser.add_argument(
        "--out-dir",
        default="/nfs/bench/faas_hybrid_etl_v1/aggregated",
...
def build_run_summary(
    ...
    for stack, run_id in keys:
        ...
        row: Dict[str, Any] = {
            ...
            "total_assigned_shards": sum(int(r.get("assigned_shards", 0) or 0) for r in parts),
        ...
        for part in parts:
            ...
            row[f"part_{pid:02d}_assigned_shards"] = part.get("assigned_shards")
        ...
        row.update(stats((r.get("input_bytes") for r in funcs), "input_bytes"))
        row.update(stats((r.get("input_record_count") for r in funcs), "input_record_count"))
        row.update(stats((r.get("kept_record_count") for r in funcs), "kept_record_count"))
        row.update(stats((r.get("sum_value") for r in funcs), "sum_value"))
        row.update(stats((r.get("sum_weighted_value") for r in funcs), "sum_weighted_value"))
        row.update(stats((r.get("total_token_count") for r in funcs), "total_token_count"))
        row.update(stats((r.get("total_unique_token_count") for r in funcs), "total_unique_token_count"))
        row.update(stats((r.get("total_bigram_count") for r in funcs), "total_bigram_count"))
        row.update(stats((r.get("total_char_count") for r in funcs), "total_char_count"))

        job_shards = {safe_int(r.get("shard_id")) for r in jobs}
        job_shards = {x for x in job_shards if x is not None}
        func_shards = {safe_int(r.get("shard_id")) for r in funcs}
        func_shards = {x for x in func_shards if x is not None}

        row["job_shard_rows"] = len(job_shards)
        row["function_shard_rows"] = len(func_shards)
        row["job_function_shard_match"] = (job_shards == func_shards) if jobs or funcs else None
...
def main() -> None:
    ...
    write_csv(out_dir / "benchmark2_run_summary.csv", run_rows)
    write_csv(out_dir / "benchmark2_part_summary.csv", part_rows)
    write_csv(out_dir / "benchmark2_job_latencies.csv", job_rows)
    write_csv(out_dir / "benchmark2_function_times.csv", func_rows)

    print(f"Wrote {len(run_rows)} rows to {out_dir / 'benchmark2_run_summary.csv'}")
    print(f"Wrote {len(part_rows)} rows to {out_dir / 'benchmark2_part_summary.csv'}")
    print(f"Wrote {len(job_rows)} rows to {out_dir / 'benchmark2_job_latencies.csv'}")
    print(f"Wrote {len(func_rows)} rows to {out_dir / 'benchmark2_function_times.csv'}")
