...
NODE_1="${NODE_1:-k8s-w2}"
KEEP_NAMESPACE="${KEEP_NAMESPACE:-0}"

mkdir -p "${BENCH_ROOT}/results/${STACK}/${RUN_ID}"

export K8S_CONTEXT RUN_ID BENCH_IMAGE TOTAL_INVOCATIONS BENCH_ROOT NS STACK WORKERS KEEP_NAMESPACE

PARTITION_ID=0 NODE_NAME="${NODE_0}" bash "${SCRIPT_DIR}/workload-job-faas-tiny-single-batch-k8s.sh" &
pid0=$!

PARTITION_ID=1 NODE_NAME="${NODE_1}" bash "${SCRIPT_DIR}/workload-job-faas-tiny-single-batch-k8s.sh" &
pid1=$!
